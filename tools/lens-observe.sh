...
#
# Run once:      sh lens-observe.sh
# Run overnight: nohup sh -c 'while :; do sh /root/lens-observe.sh; sleep 300; done' >/dev/null 2>&1 &
# Stop it:       pkill -f lens-observe
#
# Cost: one arp/ndp/lease read every 5 minutes. Roughly 1-3 MB per day.
